...
### Skol-provisionering — admin-invite, klasser/kurser, elevimport: KOD KLAR, EJ LIVE (2026-08-12)
Ersätter manuell SQL-seedning av skolor/admins/klasser/kurser/lärare/elever med riktiga in-app-flöden. **Inbjudan:** `lib/admin/invite-user.ts` (`inviteUserCore`, service-role `auth.admin.inviteUserByEmail`) delas av enstaka inbjudan (`inviteUser`-action, `InviteUserForm` på `/admin/anvandare`) och bulk-CSV (`importStudents`, `ImportStudentsForm`). Trigger `handle_new_auth_user` läser nu `role`/`school_id` ur inbjudningsmetadata istället för att alltid defaulta till student/null. **Säkerhetsfynd under bygget:** `createSchool`/`getAdminSchools` gick mot RLS som strukturellt aldrig kunde lyckas för en ny skola (id ≠ current_school_id()) — flyttade till service-role-klienten. Det avslöjade i sin tur att `/admin/skolor`, `/admin/crm`, `/admin/intresse` bara gatades på `role==='admin'`, utan skillnad mellan Elevante-personal och en kunds egen admin — ny `profiles.is_staff`-flagga (migration `20260811130000_admin_staff_flag.sql`, **måste sättas manuellt per Elevante-konto i prod**, ingen UI för det) gatar nu de tre sidorna, `createSchool`, `crm.ts`, och bootstrap-grenen i `inviteUser` (en admin bjuder bara in en ny skolas första admin om skolan saknar admin OCH den bjudande är staff). **Klasser/kurser:** `/admin/klasser` (delad route med lärarvyn, grenar på roll) + ny `/admin/kurser` — RLS tillät redan admin-skrivning skol-scopat så inga fler migrationer krävdes. Radering blockeras om klass/kurs har lektioner (även arkiverade — `ON DELETE CASCADE` skulle tysta radera transkript/chatthistorik annars). **Bulk-import:** CSV (`full_name,email,class_name`) taket sattes till 40 rader efter att kodgranskning hittade att 200 rader kunde slå i Vercels `maxDuration: 30`; `already-exists`-grenen läker nu en avbruten import (kopplar befintlig profil till klass via `upsert` istället för att bara hoppa över — annars permanent okopplade konton vid omuppladdning). Alla 17+2 uppgifter subagent-drivna med tvåstegsgranskning (spec + kodkvalitet); flera kritiska fynd (RLS-bypass, timeout-risk, permanent orphaning) fixades i granskningsloopar innan commit. **Två prod-aktiveringssteg innan detta är live:** (1) applicera migrationerna `20260811120000_school_provisioning.sql` + `20260811130000_admin_staff_flag.sql` mot `msqfuywpbrteyrzjggsw` — görs INTE automatiskt, kräver explicit godkännande; (2) kör `UPDATE profiles SET is_staff = true WHERE email = '...'` för varje känt Elevante-konto direkt efter migration 2, annars låses alla ute från `/admin/skolor`/`/admin/crm`/`/admin/intresse`. Ingen live-verifiering (Task 14, Task 17 steg 2–3) har körts än — kräver punkt 1–2 först. Plan i `docs/superpowers/plans/2026-08-11-school-provisioning.md`.
