...
Supabase Auth via @supabase/ssr i `apps/web/lib/supabase/`. RLS på alla tabeller, helpers via security definer (`public.current_school_id()`, `public.current_user_role()`). Login/signup/signOut Server Actions. `proxy.ts` refreshar session + skyddar `/app/*`. Role-baserad redirect i `/app/page.tsx`. Admin CSV-upload för timeslots + `GET /api/schedule`. 30 rutter totalt.

**2026-05-14:** Flyttat från delad Bokmassan-databas (`elevante`-schema) till dedikerat Supabase-projekt **`msqfuywpbrteyrzjggsw`** i eu-central-2 (Zurich). Alla tabeller bor nu i `public`-schemat. Migrations bor i `supabase/migrations/` (`20260514183000_initial_schema.sql` + `20260514184500_hardening.sql`). Helper-funktionen som tidigare hette `elevante.current_role()` heter nu `public.current_user_role()` för att undvika kollision med Postgres-inbyggd `current_role()`.
