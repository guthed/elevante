...
export function ClassTestRunner({ testId, questions, locale }: Props) {
  const sv = locale === 'sv';
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await submitClassTest(testId, payload);
      if (result.ok) {
        router.refresh();
      } else {
        setError(true);
      ...
      <div className="sticky bottom-0 -mx-4 border-t border-[var(--color-sand)] bg-[var(--color-canvas)]/95 px-4 py-4 backdrop-blur">
        ...
        {error ? (
          <p role="alert" className="mb-2 text-[0.875rem] text-[var(--color-error)]">
            {sv
              ? 'Något gick fel vid inlämningen. Försök igen.'
