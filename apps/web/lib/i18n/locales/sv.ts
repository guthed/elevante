import type { Dictionary } from '../types';

const sv: Dictionary = {
  meta: {
    siteName: 'Elevante',
    tagline: 'Minns allt du lär dig i skolan',
    description:
      'Elevante spelar in, transkriberar och minns allt som sägs i klassrummet — så att eleverna kan repetera när det passar dem.',
  },
  nav: {
    home: 'Hem',
    forSchools: 'För skolor',
    forTeachers: 'För lärare',
    forStudents: 'För elever',
    try: 'Prova',
    pricing: 'Priser',
    about: 'Om oss',
    blog: 'Blogg',
    contact: 'Kontakt',
    bookDemo: 'Boka demo',
    openMenu: 'Öppna meny',
    closeMenu: 'Stäng meny',
    switchLanguage: 'Byt språk',
  },
  footer: {
    rights: 'Alla rättigheter förbehållna.',
    madeIn: 'Byggt i Stockholm',
    navHeader: 'Navigera',
    companyHeader: 'Företag',
  },
  contact: {
    hero: {
      eyebrow: 'Kontakt',
      title: 'Hör av dig.',
      subtitle:
        'Skriv några rader så bokar vi in en halvtimme. Vi visar Elevante live och svarar på allt ni undrar.',
    },
    form: {
      nameLabel: 'Namn',
      namePlaceholder: 'Anna Andersson',
      emailLabel: 'E-post',
      emailPlaceholder: 'anna@skolan.se',
      schoolLabel: 'Skola eller organisation',
      schoolPlaceholder: 'Skolans namn',
      topicLabel: 'Ämne',
      topicOptions: {
        demo: 'Jag vill boka demo',
        pricing: 'Jag har frågor om pris',
        press: 'Press eller investerare',
        other: 'Något annat',
      },
      messageLabel: 'Meddelande',
      messagePlaceholder: 'Berätta kort vad du undrar…',
      submit: 'Skicka',
      submitting: 'Skickar…',
      success: 'Tack! Vi hör av oss inom kort.',
      errorGeneric: 'Något gick fel. Försök igen eller mejla john@elevante.se direkt.',
      errorRateLimit: 'För många försök. Vänta en minut och testa igen.',
      errorMissing: 'Fyll i alla fält innan du skickar.',
    },
    alternatives: {
      title: 'Eller maila oss direkt',
      email: 'john@elevante.se',
    },
  },
  auth: {
    login: {
      title: 'Logga in',
      subtitle: 'Fortsätt där du slutade.',
      emailLabel: 'E-post',
      passwordLabel: 'Lösenord',
      submit: 'Logga in',
      submitting: 'Loggar in…',
      noAccount: 'Har du inget konto?',
      signupLink: 'Skapa ett konto',
      errorInvalid: 'Fel e-post eller lösenord.',
      errorGeneric: 'Något gick fel. Försök igen.',
    },
    signup: {
      title: 'Skapa konto',
      subtitle: 'Bara för skolor med Elevante-avtal.',
      nameLabel: 'Ditt namn',
      emailLabel: 'E-post',
      passwordLabel: 'Lösenord',
      submit: 'Skapa konto',
      submitting: 'Skapar…',
      hasAccount: 'Har du redan ett konto?',
      loginLink: 'Logga in',
      confirmSent: 'Kolla din mejl — vi har skickat en bekräftelselänk.',
      errorGeneric: 'Något gick fel. Försök igen.',
      errorWeakPassword: 'Lösenordet är för svagt. Minst 8 tecken.',
      errorEmailTaken: 'E-postadressen är redan registrerad.',
    },
    signOut: 'Logga ut',
  },
  app: {
    landing: {
      title: 'Välj vy',
      subtitle:
        'Rollväxlare för utveckling. När autentisering är på plats i Fas 2 väljs rollen automatiskt utifrån ditt konto.',
      open: 'Öppna',
    },
    roleTitles: {
      student: 'Elev',
      teacher: 'Lärare',
      admin: 'Administratör',
    },
    topbar: {
      search: 'Sök i lektioner…',
      notifications: 'Notifikationer',
      userMenu: 'Din profil',
      signOut: 'Logga ut',
    },
    sidebar: {
      student: {
        overview: 'Översikt',
        lessons: 'Mina lektioner',
        chat: 'Fråga Elevante',
        library: 'Bibliotek',
        examPrep: 'Testa dina kunskaper',
        training: 'Plugga',
        learnerProfile: 'Din lärprofil',
        classTests: 'Klassprov',
      },
      teacher: {
        overview: 'Översikt',
        classes: 'Mina klasser',
        lessons: 'Lektioner',
        materials: 'Material',
        sharedTests: 'Delade prov',
        classTests: 'Klassprov',
      },
      admin: {
        overview: 'Översikt',
        schools: 'Skolor',
        classes: 'Klasser',
        courses: 'Kurser',
        users: 'Användare',
        schedule: 'Schema',
        stats: 'Statistik',
      },
    },
    mobileNav: {
      student: {
        overview: 'Hem',
        library: 'Bibliotek',
        chat: 'Fråga',
        examPrep: 'Testa',
        training: 'Plugga',
        learnerProfile: 'Profil',
        classTests: 'Prov',
      },
      teacher: {
        overview: 'Översikt',
        classes: 'Klasser',
        lessons: 'Lektioner',
        sharedTests: 'Prov',
        classTests: 'Prov',
      },
      admin: {
        overview: 'Översikt',
        schools: 'Skolor',
        classes: 'Klasser',
        courses: 'Kurser',
        users: 'Elever',
        schedule: 'Schema',
        stats: 'Statistik',
      },
    },
    navDescriptions: {
      student: {
        overview: 'Dagens lektioner och din senaste chatt',
        library: 'Alla dina lektioner, sökbara',
        chat: 'Ställ frågor om en lektion',
        examPrep: 'Gör ett övningsprov och se var du står',
        training: 'Flashcards och kunskapskollar',
        learnerProfile: 'Dina styrkor och utvecklingsområden',
        classTests: 'Prov från dina lärare',
      },
      teacher: {
        overview: 'Veckans insikt och dagens lektioner',
        classes: 'Dina klasser och elever',
        lessons: 'Inspelade lektioner och transkript',
        sharedTests: 'Prov som elever delat med dig',
        classTests: 'Skapa och rätta prov för dina klasser',
      },
      admin: {
        overview: 'Nyckeltal för skolan',
        schools: 'Hantera skolor',
        classes: 'Skapa och hantera klasser',
        courses: 'Kurser och lärartilldelning',
        users: 'Hantera roller och konton',
        schedule: 'Ladda upp lektionsschema',
        stats: 'Användning och aktivitet',
      },
    },
    account: {
      navLabel: 'Konto',
      title: 'Konto',
      subtitle: 'Hantera dina uppgifter, lösenord och språk.',
      profileHeading: 'Profil',
      avatarUpload: 'Ladda upp bild',
      avatarReplace: 'Byt bild',
      avatarRemove: 'Ta bort bild',
      avatarUploading: 'Laddar upp…',
      avatarRemoving: 'Tar bort…',
      avatarHint: 'JPG, PNG eller WEBP. Max 5 MB.',
      avatarErrorInvalid: 'Filen måste vara en bild (JPG, PNG eller WEBP).',
      avatarErrorTooLarge: 'Bilden är för stor. Max 5 MB.',
      avatarErrorGeneric: 'Något gick fel. Försök igen.',
      nameLabel: 'Namn',
      emailLabel: 'E-post',
      emailHint: 'E-postadressen hanteras av din skola och går inte att ändra här.',
      saveName: 'Spara namn',
      saving: 'Sparar…',
      nameSaved: 'Namnet är sparat.',
      securityHeading: 'Säkerhet',
      newPasswordLabel: 'Nytt lösenord',
      confirmPasswordLabel: 'Bekräfta lösenord',
      savePassword: 'Byt lösenord',
      passwordSaved: 'Lösenordet är uppdaterat.',
      languageHeading: 'Språk',
      errorInvalid: 'Fyll i ett namn.',
      errorWeak: 'Lösenordet måste vara minst 8 tecken.',
      errorMismatch: 'Lösenorden matchar inte.',
      errorGeneric: 'Något gick fel. Försök igen.',
    },
    pages: {
      student: {
        overview: {
          title: 'Välkommen tillbaka',
          subtitle:
            'Här hamnar alla lektioner du varit med på — så fort läraren laddat upp dem.',
          emptyTitle: 'Inga lektioner ännu',
          emptyBody:
            'Så fort din lärare trycker REC börjar Elevante spara åt dig. Kom tillbaka imorgon.',
          recentLessonsHeading: 'Senaste lektionerna',
          coursesHeading: 'Dina kurser',
          chatCta: 'Fråga Elevante',
          openLibrary: 'Öppna biblioteket',
          askEyebrow: 'Fråga Elevante',
          askTitle: 'Ställ en fråga om en lektion',
          askPlaceholder: 'Skriv din fråga…',
          askCta: 'Fråga',
          askExample1: 'Sammanfatta dagens lektion',
          askExample2: 'Förhör mig inför provet',
          continueHeading: 'Fortsätt där du slutade',
          continueLabel: 'Senaste chatten',
          continueCta: 'Öppna chatten',
          examQuickHeading: 'Prov på gång?',
          examQuickCta: 'Testa dina kunskaper',
        },
        library: {
          title: 'Bibliotek',
          subtitle: 'Alla lektioner du har tillgång till.',
          empty: 'Ditt bibliotek är tomt. Lektioner dyker upp här efterhand.',
          allCourses: 'Alla kurser',
          recordedLabel: 'Inspelad',
          notRecorded: 'Ej inspelad ännu',
        },
        lessonDetail: {
          back: '← Biblioteket',
          metaCourse: 'Kurs',
          metaTeacher: 'Lärare',
          metaRecorded: 'Inspelad',
          notRecorded: 'Ej inspelad ännu',
          transcriptHeading: 'Transkribering',
          transcriptPending:
            'Transkribering är inte klar ännu. Du kan ändå ställa frågor om lektionen.',
          materialsHeading: 'Material',
          materialsEmpty: 'Inget material uppladdat.',
        },
        chat: {
          title: 'Fråga Elevante',
          subtitle:
            'Ställ frågor om dina lektioner. Elevante svarar bara på det din lärare faktiskt sa — och visar varifrån svaret kommer.',
          openLessonChat: 'Chatta om denna lektion',
          startCourseChat: 'Chatta om kursen',
          pickCourse: 'Välj kurs',
          pickLesson: 'Välj lektion',
          empty: 'Ingen chat ännu — skriv en fråga nedan.',
          inputPlaceholder: 'Vad vill du veta?',
          send: 'Skicka',
          sending: 'Skickar…',
          assistantTyping: 'Elevante skriver…',
          streamError: 'Svaret kunde inte hämtas. Kontrollera uppkopplingen och försök igen.',
          guardrailNotice: 'Det togs inte upp på den här lektionen.',
          sourcesHeading: 'Källor',
          sourceFromLesson: 'Från lektionen',
          sourcesShowMore: 'Visa fler källor',
          sourcesShowLess: 'Visa färre',
          mockNotice:
            'Mockat svar — riktig RAG aktiveras i Fas 6 när KB-Whisper-pipelinen är på plats.',
          newChat: 'Ny chat',
          historyHeading: 'Tidigare chattar',
          noHistory: 'Inga tidigare chattar ännu.',
        },
      },
      teacher: {
        overview: {
          title: 'Dina klasser',
          subtitle: 'Översikt över dina kurser, lektioner och material.',
          emptyTitle: 'Inga klasser kopplade ännu',
          emptyBody:
            'Admin-teamet kopplar ditt schema. Så fort det är klart dyker dina klasser upp här.',
          classesHeading: 'Klasser',
          coursesHeading: 'Kurser',
          recentLessonsHeading: 'Senaste lektionerna',
          studentsCount: 'elever',
          lessonsCount: 'lektioner',
          heroEyebrow: 'Veckans insikt',
          heroCtaLabel: 'Se vad de undrar',
          heroQuestionWord: 'frågor',
          heroConceptPrefix: 'fastnar på',
          quickHeading: 'Snabbt',
          quickLessons: 'Mina lektioner',
          quickTests: 'Delade prov',
          chipsCourses: 'kurser',
          chipsStudents: 'elever',
          chipsClasses: 'klasser',
          chipsCourse: 'kurs',
          chipsStudent: 'elev',
          chipsClass: 'klass',
        },
        classes: {
          title: 'Klasser',
          subtitle: 'Klasserna du undervisar.',
          empty: 'Inga klasser kopplade till dig ännu.',
          year: 'Årskurs',
          studentsLabel: 'elever',
        },
        classDetail: {
          back: '← Alla klasser',
          membersHeading: 'Elever',
          membersEmpty: 'Inga elever är kopplade till klassen ännu.',
          coursesHeading: 'Kurser',
          lessonsHeading: 'Senaste lektionerna',
        },
        lessons: {
          title: 'Lektioner',
          subtitle: 'Alla dina inspelade och planerade lektioner.',
          empty:
            'Inga lektioner ännu. Tryck REC i mobil-appen så börjar det fyllas på här.',
          dateLabel: 'Datum',
          courseLabel: 'Kurs',
          classLabel: 'Klass',
          statusLabel: 'Status',
          delete: 'Radera lektion',
          deleted: 'Lektionen raderad',
          undo: 'Ångra',
          deleteError: 'Kunde inte radera lektionen',
        },
        lessonDetail: {
          back: '← Alla lektioner',
          metaCourse: 'Kurs',
          metaClass: 'Klass',
          metaTeacher: 'Lärare',
          metaRecorded: 'Inspelad',
          notRecorded: 'Ej inspelad ännu',
          materialsHeading: 'Material',
          materialsEmpty: 'Inget material uppladdat ännu.',
          uploadLabel: 'Lägg till material (PDF, bild, dokument)',
          uploadHint: 'Max 500 MB. PDF, PNG, JPG, DOCX, PPTX, XLSX, TXT.',
          uploadSubmit: 'Ladda upp',
          uploading: 'Laddar upp…',
          uploadSuccess: 'Materialet är sparat.',
          uploadError: 'Något gick fel vid uppladdningen.',
          uploadTooLarge: 'Filen är för stor (max 500 MB).',
          uploadBadType: 'Filtypen stöds inte.',
          transcriptHeading: 'Transkribering',
          transcriptPending: 'Väntar på inspelning',
          transcriptProcessing: 'Transkriberas just nu',
          transcriptReady: 'Klar',
          transcriptFailed: 'Misslyckades',
          transcriptComingSoon:
            'Transkribering aktiveras i Fas 6 när KB-Whisper-pipelinen är på plats.',
        },
        statuses: {
          pending: 'Väntar',
          processing: 'Pågår',
          ready: 'Klar',
          failed: 'Fel',
        },
      },
      admin: {
        overview: {
          title: 'Administration',
          subtitle: 'Hantera skolor, användare, scheman och statistik.',
          emptyTitle: 'Inga skolor tillagda',
          emptyBody: 'Börja med att lägga till en skola och ladda upp dess schema.',
          schoolsLabel: 'skolor',
          studentsLabel: 'elever',
          teachersLabel: 'lärare',
          lessonsLabel: 'lektioner',
          transcribedLabel: 'transkriberade',
          recentLessonsHeading: 'Senaste lektionerna',
          quickActionsHeading: 'Snabbåtgärder',
          uploadSchedule: 'Ladda upp schema',
          manageUsers: 'Hantera användare',
          manageSchools: 'Hantera skolor',
        },
        schedule: {
          title: 'Schema',
          subtitle:
            'Ladda upp en CSV med lektioner (kurs, klass, dag, starttid, sluttid, sal).',
          uploadLabel: 'Välj CSV-fil',
          uploadHint:
            'Rubriker: course_code, class_name, day, start_time, end_time, room',
          uploadSubmit: 'Ladda upp',
          uploading: 'Laddar upp…',
          termStartLabel: 'Terminen börjar',
          termEndLabel: 'Terminen slutar',
          termEndHint: 'Lämna tomt om schemat gäller tills vidare.',
          success: 'Schemat är uppdaterat.',
          successDetail: '{created} nya pass, {updated} uppdaterade, {skipped} överhoppade.',
          unmappedTeachers:
            'Lärare utan konto i Elevante — deras pass syns inte i lärarappen förrän de kopplats:',
          errorInvalid: 'CSV-filen är felformaterad. Kolla rubrikraden.',
          errorGeneric: 'Något gick fel vid uppladdning.',
          teacherMapTitle: 'Lärare i schemat',
          teacherMapEmpty: 'Inga lärare i schemat än. Ladda upp ett schema först.',
          teacherMapHint:
            'Schemat känner bara till lärarnas namn. Koppla varje namn till ett konto — annars syns inte lektionerna i lärarappen.',
          teacherMapUnmapped: 'Inte kopplad',
          teacherMapNameColumn: 'I schemat',
          teacherMapAccountColumn: 'Elevante-konto',
          teacherMapSlotsColumn: 'Pass',
          teacherMapPlaceholder: 'Välj konto…',
          teacherMapSave: 'Spara',
          teacherMapSaving: 'Sparar…',
          teacherMapSaved: 'Kopplad — {slots} pass och {courses} kurser uppdaterade.',
          teacherMapSuggestion: 'Förslag: {name}',
          teacherMapError: 'Kunde inte spara kopplingen.',
          tableEmpty: 'Inget schema uppladdat än.',
          dayColumn: 'Dag',
          startColumn: 'Start',
          endColumn: 'Slut',
          roomColumn: 'Sal',
        },
        users: {
          title: 'Användare',
          subtitle: 'Hantera elever, lärare och administratörer i din skola.',
          empty: 'Inga användare i din skola ännu.',
          nameColumn: 'Namn',
          emailColumn: 'E-post',
          roleColumn: 'Roll',
          updateRole: 'Uppdatera',
          updating: 'Uppdaterar…',
          updateSuccess: 'Rollen uppdaterad.',
          updateError: 'Kunde inte uppdatera rollen.',
          remove: {
            button: 'Ta bort',
            removing: 'Tar bort…',
            confirm: 'Ta bort den här användaren permanent? Går inte att ångra.',
            errorLastAdmin: 'Kan inte ta bort skolans enda admin.',
            errorGeneric: 'Kunde inte ta bort användaren.',
          },
          invite: {
            heading: 'Bjud in användare',
            nameLabel: 'Namn',
            emailLabel: 'E-post',
            roleLabel: 'Roll',
            roleStudent: 'Elev',
            roleTeacher: 'Lärare',
            roleAdmin: 'Administratör',
            submit: 'Skicka inbjudan',
            sending: 'Skickar…',
            success: 'Inbjudan skickad till {email}.',
            errorExists: 'Det finns redan ett konto med den e-postadressen.',
            errorGeneric: 'Kunde inte skicka inbjudan.',
          },
          import: {
            heading: 'Importera användare',
            fileLabel: 'CSV-fil',
            fileHint: 'Rubriker: full_name, email, class_name, valfri role (student/teacher, standard student). Max 3000 rader — ladda upp hela skolan på en gång.',
            submit: 'Importera',
            importing: 'Importerar…',
            successCount: '{count} elever inbjudna.',
            errorGeneric: 'Något gick fel vid importen.',
            reasonInvalidRow: 'Ofullständig rad (namn, e-post eller klass saknas)',
            reasonAlreadyExists: 'Kontot finns redan',
            reasonClassLinkFailed: 'Kunde inte koppla till klassen',
            reasonGeneric: 'Okänt fel',
          },
          detail: {
            back: 'Tillbaka till användare',
            createdLabel: 'Skapad',
            classesHeading: 'Klasser',
            classesEmpty: 'Inte kopplad till någon klass.',
            coursesHeading: 'Kurser',
            coursesEmpty: 'Inte tilldelad någon kurs.',
            resendHeading: 'Inloggningslänk',
            resendHint: 'Skicka en ny länk om den gamla har slutat fungera eller redan använts.',
            resendSubmit: 'Skicka ny länk',
            resendSending: 'Skickar…',
            resendSuccess: 'Ny länk skickad till {email}.',
            resendErrorGeneric: 'Kunde inte skicka länken.',
          },
          massInvite: {
            heading: 'Bjud in flera på en gång',
            roleLabel: 'Roll',
            roleStudent: 'Elever',
            roleTeacher: 'Lärare',
            roleAdmin: 'Administratörer',
            classLabel: 'Klass',
            classPlaceholder: 'Välj klass…',
            entriesLabel: 'Namn och e-post',
            entriesHint: 'Klistra in en person per rad. Namnet gissas fram ur e-postadressen om du bara skriver mejlet.',
            entriesFormatHint: 'En rad per person, t.ex. "Anna Andersson anna@skola.se" eller bara mejladressen.',
            entriesPlaceholder: 'Anna Andersson anna@skola.se\nErik Svensson erik@skola.se',
            submit: 'Skicka inbjudningar',
            sending: 'Skickar…',
            successCount: '{count} inbjudningar skickade.',
            errorGeneric: 'Kunde inte skicka inbjudningarna.',
            reasonInvalidRow: 'Kunde inte tolka raden',
            reasonAlreadyExists: 'Kontot finns redan',
            reasonClassLinkFailed: 'Kunde inte koppla till klassen',
            reasonGeneric: 'Okänt fel',
          },
        },
        schools: {
          title: 'Skolor',
          subtitle: 'Översikt över skolor i Elevante. Lägg till en ny vid behov.',
          empty: 'Inga skolor ännu. Skapa den första nedan.',
          createTitle: 'Lägg till skola',
          nameLabel: 'Namn',
          slugLabel: 'URL-slug',
          slugHint: 'Ett kort, unikt namn utan mellanslag eller å/ä/ö — bara internt, syns inte för elever eller lärare.',
          countryLabel: 'Land (ISO)',
          createSubmit: 'Skapa',
          creating: 'Skapar…',
          createSuccess: 'Skolan är skapad.',
          createError: 'Kunde inte skapa skolan.',
          createdLabel: 'Skapad',
          bootstrapAdmin: {
            heading: 'Bjud in första admin',
            nameLabel: 'Namn',
            emailLabel: 'E-post',
            submit: 'Bjud in',
            sending: 'Skickar…',
            success: 'Inbjudan skickad till {email}.',
            errorExists: 'Det finns redan ett konto med den e-postadressen.',
            errorGeneric: 'Kunde inte skicka inbjudan.',
          },
          staff: {
            heading: 'Elevante-personal',
            hint: 'Ge en befintlig användare rättighet att skapa skolor och bootstrapa admins. Bara Elevante-personal ska ha den här flaggan.',
            emailLabel: 'E-post',
            grantSubmit: 'Ge behörighet',
            granting: 'Uppdaterar…',
            success: 'Behörigheten uppdaterad.',
            errorNotFound: 'Ingen användare med den e-postadressen.',
            errorSelf: 'Kan inte återkalla din egen behörighet.',
            errorGeneric: 'Kunde inte uppdatera behörigheten.',
            listEmpty: 'Ingen Elevante-personal registrerad.',
            revoke: 'Återkalla',
            revoking: 'Återkallar…',
          },
          detail: {
            back: 'Tillbaka till skolor',
            studentsLabel: 'elever',
            teachersLabel: 'lärare',
            adminsLabel: 'admins',
            classesHeading: 'Klasser',
            classesEmpty: 'Inga klasser ännu.',
            coursesHeading: 'Kurser',
            coursesEmpty: 'Inga kurser ännu.',
            adminsHeading: 'Administratörer',
            adminsEmpty: 'Ingen admin ännu.',
          },
        },
        classes: {
          title: 'Klasser',
          subtitle: 'Skapa och hantera klasser i din skola.',
          empty: 'Inga klasser ännu. Skapa den första till höger.',
          studentsLabel: 'elever',
          lessonsLabel: 'lektioner',
          delete: 'Ta bort',
          deleting: 'Tar bort…',
          deleteErrorHasLessons: 'Klassen har inspelade lektioner och kan inte tas bort.',
          deleteErrorGeneric: 'Kunde inte ta bort klassen.',
          createTitle: 'Ny klass',
          nameLabel: 'Namn',
          yearLabel: 'Årskurs',
          yearHint: 'Valfritt, 1–12.',
          createSubmit: 'Skapa',
          creating: 'Skapar…',
          createSuccess: 'Klassen är skapad.',
          createErrorDuplicate: 'En klass med det namnet finns redan.',
          createErrorGeneric: 'Kunde inte skapa klassen.',
        },
        courses: {
          title: 'Kurser',
          subtitle: 'Skapa kurser och tilldela lärare i din skola.',
          empty: 'Inga kurser ännu. Skapa den första till höger.',
          delete: 'Ta bort',
          deleting: 'Tar bort…',
          deleteErrorHasLessons: 'Kursen har inspelade lektioner och kan inte tas bort.',
          deleteErrorGeneric: 'Kunde inte ta bort kursen.',
          teachersHeading: 'Lärare på kursen',
          noTeachers: 'Ingen lärare tilldelad ännu.',
          removeTeacher: 'Ta bort lärare',
          pickTeacher: 'Välj lärare…',
          assign: 'Lägg till',
          assigning: 'Lägger till…',
          assignError: 'Kunde inte lägga till läraren.',
          createTitle: 'Ny kurs',
          codeLabel: 'Kurskod',
          codeHint: 'En kort kod du själv väljer, t.ex. MA3 eller BIO1. Används i scheman och listor.',
          nameLabel: 'Namn',
          createSubmit: 'Skapa',
          creating: 'Skapar…',
          createSuccess: 'Kursen är skapad.',
          createErrorDuplicate: 'En kurs med den koden finns redan.',
          createErrorGeneric: 'Kunde inte skapa kursen.',
        },
        stats: {
          title: 'Statistik',
          subtitle: 'Aktivitet i din skola.',
          weeklyHeading: 'Lektioner senaste 7 dagarna',
          statusHeading: 'Transkriberings-status',
          activityHeading: 'Aktiva användare',
          noData: 'Inga data ännu. Statistik fylls på allteftersom.',
        },
      },
    },
    common: {
      comingSoon: 'Kommer i nästa fas',
      loading: 'Laddar…',
    },
    klassprov: {
      newTest: 'Nytt prov',
      title: 'Titel',
      pickClass: 'Välj klass',
      pickLessons: 'Välj lektioner',
      questionCount: 'Antal frågor',
      closed: 'Stängda',
      open: 'Öppna',
      reasoning: 'Resonerande',
      generate: 'Generera utkast',
      publish: 'Publicera till klassen',
      regenerate: 'Regenerera',
      takeTest: 'Gör provet',
      submit: 'Lämna in',
      awaitingReview: 'Inlämnat — väntar på lärarens granskning',
      release: 'Släpp till elev',
      released: 'Resultat släppt',
      pendingReview: 'väntar på granskning',
      notStarted: 'Inte börjat',
      overallFeedback: 'Helhetskommentar',
      points: 'Poäng',
      save: 'Spara',
      remove: 'Ta bort',
    },
  },
};

export default sv;
